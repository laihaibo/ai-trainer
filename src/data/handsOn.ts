 ...
 * pitfalls = 考点与易错点；
 * regression = 回归分析专题（仅涉回归模块提供，要点源自 2.人工智能模型训练实践.pdf 与
 *   逻辑回归模型评价指标分析.pdf/docx 中已核实的内容）；
 * highScoreGuide = 如何生成高分答案（阅卷得分动作清单，要求均取自各实训资料的完成标准）。
 */
...
export const HANDS_ON: HandsOnModule[] = [
  /* ==================== 模块一：Python 编程 ==================== */
  {
    ...
    highScoreGuide: [
      '按题面补全，不改动既有代码结构：填空只做「去掉行首 #」与「补下划线代码」，步骤顺序保持 读取 → head 预览 → describe → 归一化 → corr → heatmap 不变。',
      '输出行数对齐题意：题干写"前 10 行预览"就交 df.head(10)，写"前 5 行"用 df.head()——输出与题目要求不一致会直接扣分。',
      '归一化对象别错：test2 只对 numeric_cols 数值列做 MinMaxScaler；转回 DataFrame 必须带 columns=df.columns，列名丢失会导致后续 corr 结果可读性差。',
      '图表配置三件套齐全：rcParams 中文字体两行（防乱码与负号消失）+ plt.tight_layout() + plt.show()；确认热力图数值（annot=True, fmt=".2f"）完整可见后再截图。',
      '答案文档一次成型：把源代码窗口截图、运行结果截图（Console 输出 + 每张图）分别粘贴进答题 docx 并按小题编号排列；先跑通全流程再逐项截图，避免中途报错的残缺截图。',
    ],
  },
  /* ==================== 模块二：模型构建 ==================== */
  {
    ...
    regression: [
      {
        title: '回归分析是什么（先定位题型再动笔）',
        points: [
          '回归分析：研究被解释变量（因变量 y）与一个或多个解释变量（自变量 x）之间数量关系的统计方法，用途是预测与业务归因解释——本题即"财政收入增速等自变量 → 增长是否达标/水平预测"。',
          '题型二分法：y 为连续数值（如财政收入规模）→ 线性/多元回归，评价看 R² 系列与 RMSE；y 为 0/1 二分类（如增长是否达标）→ 逻辑回归，评价看混淆矩阵、precision/recall/F1 与 ROC-AUC 一族。',
          '两类题共用同一作答骨架：读数探查（head/describe/info）→ 选特征定变量 → 拟合训练 → 输出指标 → 逐条指标下结论。区别只在指标体系（见下一专题）。',
        ],
      },
      {
        title: '多元线性回归质量评价：五层次递进法',
        points: [
          '第 1 层 整体显著性：F 检验的 p 值 < 0.05，模型整体有效；不显著则后续讨论无意义。（对应 JASP 输出表的 ANOVA 部分。）',
          '第 2 层 解释力度：R² 越接近 1 越好；多元回归以调整R² 更客观（避免加变量带来的虚高），配合 RMSE 说明误差绝对量级。',
          '第 3 层 变量层面：逐个系数 β 看符号是否符合业务直觉，t 检验 p < 0.05 的变量才保留；不显著的变量要给出去留建议。',
          '第 4 层 共线性诊断：条件指数 > 30 判定存在严重共线性；方差比例 > 0.5 可锁定涉及的变量；JASP 中也可看 VIF（>10 严重）与容忍度。',
          '第 5 层 残差分析：标准化残差直方图呈钟形、Q-Q 图散点贴合对角直线为合格；明显偏离时统计推断不可靠，答题要写"模型假设不成立，需改进"而非硬套结论。',
          '多元 vs 一元的高频问答："加入新变量是否提升模型？"的标准答法是比较加入前后的调整R² 与新系数 p 值，而非只看拟合优度上升。',
        ],
      },
      {
        title: '逻辑回归评价体系（JASP 指标对照）',
        points: [
          '拟合优劣：Deviance 越小拟合越好；AIC/BIC 取小者更优，两者只在同一样本下比较，且 AIC 差值 > 2 才算实质差异；BIC 惩罚更重、偏向简单模型。',
          '整体显著性：似然比卡方检验 + 对应 p 值 < 0.05，说明模型整体优于空模型。',
          '解释力度：伪R² 三兄弟（McFadden / Nagelkerke / Cox-Snell）只能各自解读、不可跨类别直接比较大小。',
          '分类能力：混淆矩阵（总体准确率 Accuracy）+ ROC 曲线与 AUC（越接近 1 区分力越强，0.5 等于随机猜）；样本不平衡时优先汇报 AUC / F1。',
        ],
      },
      {
        title: '回归类简答题万能句式（背了就能用）',
        points: [
          '"在 α = 0.05 水平上，模型整体显著性检验 p = ___ (< 0.05)，模型整体显著。"',
          '"调整R² = ___，说明所选解释变量共同解释了约 ___% 的因变量变动。"（多元模型务必用调整R² 作答。）',
          '"___ 变量系数 β = ___（p < 0.05），与 y 呈正/负相关，与业务预期一致/不一致。"',
          '"条件指数最大值为 ___ (> 30)，提示 ___ 与 ___ 存在共线性，建议删除或合并其一后再估计。"',
          '"残差直方图近似正态、Q-Q 图散点基本贴合直线，回归假设基本满足，模型可用于预测/需进一步修正。"（按图形实际二选一。）',
        ],
      },
    ],
    highScoreGuide: [
      '答案写成「小报告」结构：任务目标一句话 → 特征选择及理由（剔除 year 标识列；选取收入增速、财政自给率作解释变量）→ 关键参数（test_size=0.3、random_state=42 保证可复现）→ 指标表格 → 每个指标配一句业务解读 → 结论与建议。',
      '指标必须带解读，不能只贴数字：accuracy 说总体判对比例；precision 预测为正里有多少真对、recall 真正例里找回了多少；不平衡数据主动指出应改看 F1 或 AUC——只报数字不解读是最常见失分点。',
      '混淆矩阵标注四格含义：注明 TN/FP/FN/TP 各是多少（sklearn 为 [[TN,FP],[FN,TP]]，行真实列预测），并从它推导 accuracy / precision / recall / F1，展示计算来源更稳。',
      'JASP 类输出按五层次逐条作答：整体显著性 p 值 → R²/调整R²/RMSE → 各系数 β 与 p 值 → 共线性诊断（写出条件指数/VIF 具体数值）→ 残差图形判断；每一层都给出"通过/不通过 + 数值依据"的明确结论。',
      '代码与截图成对出现：关键产出（数据预览、fit 训练、混淆矩阵、classification_report、ROC 曲线图）各配运行截图贴在对应小节下方，图文一致、按小题编号排列。',
    ],
  },
  /* ==================== 模块三：数据标注 ==================== */
  {
    ...
    highScoreGuide: [
      '先设路径再开标：Open Dir 选 images 目录、Change Save Dir 指 labels 目录、保存格式下拉框切到 YOLO，三项确认无误才开始画框——设置界面本身就是要截的第一张图。',
      '五张必备截图一次备齐：① 图片路径设置 ② 标注保存路径与 YOLO 格式切换界面 ③ labels 目录 50 个 txt 全览 ④ 第 1 个与第 25 个标注文件内容 ⑤ data.yaml 完整内容——均为阅卷得分点，做完逐项自查补拍。',
      '标注质量三问：类别选对了吗（Gloves / No-Gloves / Goggles / No-Goggles，No- 前缀=未佩戴）；框是否贴合目标主体；遮挡严重、目标极小的目标是否确实未乱标。',
      'txt 抽查一行：五个值空格分隔、四个坐标值都在 [0,1]、class_id 从 0 开始且 < nc；文件与图像同名仅后缀为 .txt。',
      'data.yaml 存盘前通读一遍：path / train / val / nc / names 五字段齐全，train 与 val 都填 images；核对冒号后空格、names 两空格缩进（禁 Tab）、nc: 4、类名大小写与连字符，最终文件名必须是 data.yaml 而非 data.yaml.txt。',
    ],
  },
]
