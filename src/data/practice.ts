    ...
    shots: [
      {
        ...
        answer:
          '运行模型后，模型汇总表显示：R²=0.984、调整后 R²=0.983，即 invest_hour 单独解释了 profit 约 98.4% 的变异，拟合优度极高；RMSE 从无模型时的 9.751 降至 1.259，预测误差大幅收窄。方差分析表显示 F=2892、p<.001，回归方程整体显著，线性关系成立。系数表显示：截距为 9.371（p<.001）；invest_hour 的非标准化系数 B=2.856（t=53.78、p<.001），即部门月度运营投入每增加 1 单位，部门月度利润平均增加约 2.856 单位，且这一效应高度显著；标准化系数 0.9918 接近 1，与 R² 互相印证。综上，该一元线性回归模型拟合效果优秀，运营投入对利润存在显著正向影响。',
      },
      {
        src: '1-3.png',
        ...
        answer:
          '运行多元线性回归模型后：模型汇总表显示 R²=0.996、调整后 R²=0.996（较一元模型的 0.984/0.983 进一步提升），RMSE 由 1.259 降至 0.6155；方差分析表 F=6125、p<.001，回归方程整体显著。系数表显示：market_cost 的非标准化系数 B=3.200（t 检验 p<.001，标准化系数 0.9130），即市场推广费用每增加 1 单位，部门月度利润平均增加约 3.200 单位，效应显著；但 invest_hour 的 p 值由一元模型的 <.001 变为 .251，不再显著，其标准化系数降至 0.086。共线性诊断表显示条件指数第 3 维为 76.41（大于 30 的警戒值），该维度上 invest_hour 与 market_cost 的方差比例分别为 0.999 与 0.993，说明两者之间存在强多重共线性，正是它掩盖了 invest_hour 的独立效应。残差方面，标准化残差直方图整体近似钟形（仅左端 −3.5 附近有一个孤立柱），标准化残差 Q-Q 图散点基本贴合对角线（仅一个点偏离至约 (−2.3, −3.1)），残差近似正态分布，存在个别离群样本。综上，该多元模型拟合优度更高、方程整体显著，但存在较强多重共线性，单个系数的解释需谨慎。',
      },
    ],
    verdict:
    ...
    shots: [
      {
        ...
        answer:
          '运行一元逻辑回归模型后，模型汇总表显示：Deviance 由 41.59 降至 30.42，AIC 由 43.589 降至 34.419，BIC 由 44.990 降至 37.222，加入解释变量后三者同步下降（三者均以越小越好）；ΔX²=11.170（df=1，p<.001），模型显著优于空模型。伪 R² 方面：McFadden R²=0.2686（大于 0.2，拟合良好）、Nagelkerke R²=0.4145、Cox & Snell R²=0.3109，逻辑回归不使用普通 R²，需以伪 R² 衡量解释力。系数表显示：revenue_growth_rate 的 β=0.557（Wald=6.655，p=.010），Odds Ratio=1.745，即财政收入增速每提高 1 单位，增长达标的 odds 变为原来的约 1.745 倍；截距为 −5.922（p=.012）。判别表现方面：混淆矩阵（cut-off=0.5）显示 Overall 正确率 76.67%，其中真实未达标组 12/15 判对（80.00%）、真实达标组 11/15 判对（73.33%）；Accuracy=0.7667、Sensitivity=0.7333、Precision=0.7857、AUC=0.8089（0.7~0.9 属可用/较好），ROC 曲线明显拱向左上角。综上，该一元逻辑回归模型拟合与判别表现良好，财政收入增速对增长达标有显著正向影响。',
      },
      {
        src: '2-3.png',
        ...
        answer:
          '加入 fiscal_self_sufficiency 后，多元逻辑回归结果全面优于一元模型：Deviance 由 30.42 降至 24.43，AIC 由 34.419 降至 30.433，BIC 由 37.222 降至 34.637，均进一步下降；ΔX²=17.156（p<.001），模型显著优于空模型；McFadden R² 由 0.2686 升至 0.4125，Nagelkerke R² 由 0.4145 升至 0.5807，解释力明显增强。系数表显示：revenue_growth_rate 的 β=0.584（Odds Ratio=1.793，p=.025），fiscal_self_sufficiency 的 β=0.150（Odds Ratio=1.162，p=.031），两个变量均在 p<.05 水平显著，财政自给率对增长达标同样有正向贡献。判别表现方面：混淆矩阵 Overall 正确率由 76.67% 提升至 83.33%（真实达标组判对 11→13、误判 4→2）；Accuracy=0.8333、Sensitivity=0.8667、Precision=0.8125，AUC 由 0.8089 升至 0.8844。综上，多元逻辑回归模型的拟合效果与判别能力均优于一元模型，财政收入增速与财政自给率都是影响增长达标水平的显著因素。',
      },
    ],
    verdict:
    ...
    shots: [
      {
        ...
        answer:
          '经「回归 → 广义线性模型」构建一元泊松回归模型（分布选 Poisson、链接函数 log），结果标题为「广义线性模型 / Model Summary - revenue_order」，注 H₁ includes service_hour。模型汇总表显示：Deviance 由 3.320×10⁷ 降至 2.526×10⁷，X²=7.941×10⁶（df=1，p<.001），模型显著优于空模型；AIC=2.527×10⁷、BIC=2.527×10⁷。系数表显示：service_hour 的 β=0.2913（z=2826，p<.001）。泊松回归为 log 链接，系数作用在对数尺度上，须经换算解释：e^0.2913≈1.34，即门店日均导购服务时长每增加 1 单位，期望订单量乘以约 1.34（若直接说「增加 0.29 单」则解释错误）；截距 8.163 为期望订单量对数的基线。综上，一元泊松回归模型整体显著且服务时长对订单量有显著正向影响，但 Deviance 仍高达 2.5×10⁷、AIC 与空模型差距相对有限，单一变量解释力不足，需纳入更多协变量以提升拟合效果。',
      },
      {
        src: '3-3.png',
        ...
        answer:
          '加入 display_score、flow_cost 后，多元泊松回归结果如下：模型汇总 H₁ 的 Deviance 由 2.526×10⁷ 骤降至 1038，AIC 由 2.527×10⁷ 降至 1.225×10⁴，X²=3.320×10⁷（p<.001），三变量联合几乎解释了订单量的全部系统变异，模型显著改良。系数表显示：service_hour 的 β=0.3001、display_score 的 β=0.5001、flow_cost 的 β=0.001000，三个协变量的 z 检验 p 值均小于 .001，均高度显著；按 e^β 换算，陈列评分每增加 1 单位，期望订单量约乘以 e^0.5≈1.65。共线性诊断（Multicollinearity Diagnostics）输出 Tolerance≈0.99（大于 0.1 警戒线）、VIF≈1.00~1.01（远小于 10），不存在多重共线性，模型结构健康。残差诊断图方面：标准化偏差残差与拟合值散点围绕 0 线无系统趋势，Q-Q 图散点紧贴对角线（仅左尾微弯），残差分布合理。综上，多元泊松回归模型拟合效果显著优于一元模型，三个影响因素均显著且无共线性问题，模型更稳健。',
      },
    ],
    verdict:
