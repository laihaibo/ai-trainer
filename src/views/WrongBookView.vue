...
/** 去重重练：全部（默认）或指定题目入队（跳过题库已移除的题目） */
function startReview(targetIds?: number[]): void {
  const source =
    targetIds !== undefined
      ? items.value
...
<template>
  <div>
    ...
    <template v-if="mode === 'list'">
      <div class="page-head">
        <h1>错题本</h1>
        <p class="page-sub">累计错题 {{ progress.wrongCount.value }} 道（按错误次数降序）</p>
      </div>
    ...
    <template v-else>
      <div class="quiz-head">
        <button class="btn btn-secondary" type="button" @click="exitReview">← 返回错题清单</button>
        <h2 class="quiz-title">错题重练</h2>
      ...
      <div v-if="reviewDone" class="card result-card">
        <h3>错题已全部处理</h3>
        <p class="result-hint">已掌握的题已从错题本移除，可返回清单查看。</p>
        <div class="result-actions">
          <button class="btn" type="button" @click="exitReview">返回错题清单</button>
        </div>
      </div>
