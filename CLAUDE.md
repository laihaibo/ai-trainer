...
- `src/types.ts` — 全局数据契约（Question / Topic / DailyPlan / HandsOnModule / ProgressState 等），所有共享类型只在这里定义，勿在别处重复定义
- `src/data/` — 内容数据（静态 TS，无后端）：
  - `topics.ts` — 10 主题 / 118 张速查卡片，每条标注【题库】/【实训】来源，卡片 id 与题库 topic 字段严格一致
  - `plan.ts` — 5 天冲刺日计划（8/25–8/29，8/30 考试；task id 全局唯一如 `d1t1`，用于 localStorage 打卡）
